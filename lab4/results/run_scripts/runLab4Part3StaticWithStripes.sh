...
# Runs the static stripe-enabled Part 3 flow.
#
# Usage:
#   ./scripts/runLab4Part3StaticWithStripes.sh [output_dir]
#
# Example:
#   ./scripts/runLab4Part3StaticWithStripes.sh
#   ./scripts/runLab4Part3StaticWithStripes.sh results/part3_static_with_stripes

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LAB4_DIR="$(cd "${SCRIPT_DIR}/.." && pwd)"
STATIC_TCL="${LAB4_DIR}/lab4_part3_static_with_stripes.tcl"
...
if [ "${1:-}" = "" ]; then
    OUT_DIR="${LAB4_DIR}/results/part3_static_with_stripes"
else
    if [[ "$1" = /* ]]; then
        OUT_DIR="$1"
    else
...
mkdir -p "$OUT_DIR"
CONSOLE_LOG="${OUT_DIR}/innovus_part3_static_with_stripes_console.log"

echo "=========================================="
echo "  Lab 4 Part 3 Static Run (With Stripes)"
echo "=========================================="
