...
# Runs Lab 4 Part 2 timing-driven flow.
# Usage:
#   ./scripts/runLab4Part2TD.sh

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LAB4_DIR="$(cd "${SCRIPT_DIR}/.." && pwd)"
PART2_TCL="${LAB4_DIR}/lab4_part2.tcl"
RESULTS_DIR="${LAB4_DIR}/results/timing_driven"
RUN_ID="$(date +%Y%m%d_%H%M%S)_$$"
CONSOLE_LOG="${LAB4_DIR}/output/innovus_part2_td_console_${RUN_ID}.log"
...
echo "=========================================="
echo "  Lab 4 Part 2 — Timing-Driven"
echo "=========================================="
...
INVS_FILE="${RESULTS_DIR}/s1494_timing_driven.invs"
INVS_DAT_DIR="${RESULTS_DIR}/s1494_timing_driven.invs.dat"
if [ ! -e "$INVS_FILE" ] || [ ! -d "$INVS_DAT_DIR" ]; then
    echo "ERROR: Missing required Part 2 TD artifacts:"
    echo "  $INVS_FILE"
    echo "  $INVS_DAT_DIR"
    exit 3
fi
