...
2. **Instance Terminals (oaInstTerm)**: Uses the center point of each instance's bounding box as the endpoint location.

3. **HPWL Computation**: Finds min/max x/y across all endpoint points and computes HPWL = (max_x - min_x) + (max_y - min_y).
...
3. Instance endpoints use center of instance bounding box.
4. Center-point policy is applied consistently in both Problem 1 OA HPWL and Problem 2 cached HPWL evaluation.
...
4. **Consistent endpoint model across Problem 1 and Problem 2 cache math**:
   - Both OA-based HPWL and cached HPWL use pure center-point endpoint policy.
...
- **Number of swaps**: 2
- **Execution time (single run shown)**: 0.000966 sec
- **Score (single run shown)**: 3.0285e+10

### Runtime Note

Server and load conditions cause run-to-run variation, so rerunning is recommended when collecting final score numbers.

Origin-based example variant:
- `lab3/lab3_origin.cpp` demonstrates an instance-origin endpoint model for comparison only.
- `lab3/scripts/runLab3Origin.sh` builds/runs that variant.
- This report and the primary grading flow refer to `lab3/lab3.cpp` run via `lab3/scripts/runLab3.sh`.
